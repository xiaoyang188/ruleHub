"use client";

import {
  BookOpen,
  BriefcaseBusiness,
  LogIn,
  Menu,
  Newspaper,
  Package,
  Search,
  Sparkles,
  Users,
  X,
} from "lucide-react";
import { useState } from "react";
import { LocaleLink } from "@/components/layout/locale-link";
import { ThemeToggle } from "@/components/layout/theme-toggle";
import { siteFeatures } from "@/lib/site-features";
import { cn } from "@/lib/utils";

const NAV_ITEMS = [
  { href: "/search", label: "搜索", icon: Search, highlight: true },
  { href: "/skills", label: "Skills", icon: Package },
  { href: "/creators", label: "创作者", icon: Users },
  { href: "/occupations", label: "职业技能", icon: BriefcaseBusiness },
  { href: "/vibecoding", label: "VibeCoding", icon: Sparkles },
  { href: "/vibecoding/news", label: "AI 消息", icon: Newspaper },
  { href: "/docs", label: "文档", icon: BookOpen },
] as const;

function NavPill({
  href,
  label,
  icon: Icon,
  highlight,
  onClick,
  compact,
}: {
  href: string;
  label: string;
  icon: typeof Search;
  highlight?: boolean;
  onClick?: () => void;
  compact?: boolean;
}) {
  return (
    <LocaleLink
      href={href}
      onClick={onClick}
      title={label}
      className={cn(
        "group flex shrink-0 cursor-pointer items-center gap-1.5 rounded-lg border font-medium transition-all duration-200 active:scale-95",
        compact ? "px-2.5 py-1.5 text-xs" : "px-3 py-2 text-sm",
        highlight
          ? "animate-pulse-border-primary border-primary/50 bg-card text-foreground hover:border-primary hover:bg-primary/10"
          : "border-border bg-card text-foreground hover:border-primary/50 hover:bg-muted/50"
      )}
    >
      <Icon
        className={cn(
          "shrink-0",
          compact ? "h-3.5 w-3.5" : "h-4 w-4",
          highlight ? "text-primary" : "text-blue-500"
        )}
      />
      <span className="whitespace-nowrap">{label}</span>
    </LocaleLink>
  );
}

export function Header() {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <header className="fixed left-0 right-0 top-0 z-50 border-b border-border before:absolute before:inset-0 before:-z-10 before:bg-background/80 before:backdrop-blur-sm">
      <div className="relative z-10 mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center gap-2 sm:gap-3">
          <LocaleLink
            href="/"
            className="group flex shrink-0 cursor-pointer items-center transition-opacity hover:opacity-90"
            aria-label="RuleHub home"
          >
            <div className="flex items-baseline">
              <span className="text-lg font-extrabold tracking-tight text-foreground sm:text-xl">
                rule
              </span>
              <span className="text-lg font-extrabold tracking-tight text-primary sm:text-xl">
                hub
              </span>
            </div>
          </LocaleLink>

          <nav
            aria-label="主导航"
            className="hidden min-w-0 flex-1 items-center justify-center gap-1.5 overflow-x-auto lg:flex xl:gap-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
          >
            {NAV_ITEMS.map((item) => (
              <NavPill key={item.href} {...item} compact />
            ))}
          </nav>

          <div className="ml-auto flex shrink-0 items-center gap-1.5 sm:gap-2">
            <ThemeToggle className="hidden lg:inline-flex" />

            {siteFeatures.showAuth && (
              <LocaleLink
                href="/auth/login"
                className="group hidden items-center gap-2 rounded-lg border border-primary/50 bg-card px-3 py-1.5 text-sm font-medium text-foreground transition-all duration-200 hover:border-primary hover:bg-primary/10 active:scale-95 lg:flex"
              >
                <LogIn className="h-4 w-4 text-primary" />
                登录
              </LocaleLink>
            )}

            <button
              type="button"
              className="rounded-lg p-2 text-foreground lg:hidden"
              onClick={() => setMenuOpen(!menuOpen)}
              aria-label="菜单"
              aria-expanded={menuOpen}
            >
              {menuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
            </button>
          </div>
        </div>
      </div>

      {menuOpen && (
        <div className="border-t border-border bg-background px-4 py-4 lg:hidden">
          <nav className="flex flex-col gap-2" aria-label="移动端导航">
            {NAV_ITEMS.map((item) => (
              <NavPill
                key={item.href}
                {...item}
                onClick={() => setMenuOpen(false)}
              />
            ))}
            <div className="mt-2 flex items-center justify-between rounded-lg border border-border px-3 py-2">
              <span className="text-sm text-muted-foreground">主题</span>
              <ThemeToggle />
            </div>
            {siteFeatures.showAuth && (
              <LocaleLink
                href="/auth/login"
                className="mt-2 inline-flex items-center justify-center gap-2 rounded-lg border border-primary/50 px-3 py-2.5 text-sm font-medium"
                onClick={() => setMenuOpen(false)}
              >
                <LogIn className="h-4 w-4 text-primary" />
                登录
              </LocaleLink>
            )}
          </nav>
        </div>
      )}
    </header>
  );
}
